...
import { User, Phone, Mail, MapPin, Building2, Download, FileText, ShoppingCart, Package, Clock, CheckCircle2, AlertCircle, Loader2, Wrench, Search, Video, BookOpen } from "lucide-react";
...
const OficinaDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<"hub" | "perfil" | "compras" | "catalogos" | "orcamentos" | "vistas" | "artigos" | "videos">("hub");
  ...
  const quoteStatusLabel: Record<string, string> = {
    pending: "Aguardando Resposta", answered: "Respondido", approved: "Aprovado", rejected: "Recusado",
  };
    ...
    return (
      <Layout>
        <div className="container py-16 text-center">
          ...
          <Button onClick={() => navigate("/auth")}>Cadastrar como Oficina</Button>
        </div>
      </Layout>
    );
  }
    ...
    return (
      <Layout>
        <div className="container py-16 text-center max-w-md mx-auto">
          ...
          <p className="text-muted-foreground mb-4">Seu cadastro de oficina está sendo analisado.</p>
  ...
  const tabs = [
    ...
    { id: "vistas" as const, label: "Vistas Explodidas", icon: Search },
    { id: "artigos" as const, label: "Artigos Técnicos", icon: BookOpen },
    { id: "videos" as const, label: "Vídeos", icon: Video },
  ...
  return (
    <Layout>
      ...
      <div className="container py-6">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === tab.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}>
                  ...
                  <CardContent className="flex items-center gap-4 py-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        ...
                        {tab.id === "catalogos" && `${catalogs.length} catálogos`}
                        {tab.id === "vistas" && "Diagramas de motores"}
                        {tab.id === "artigos" && "Guias técnicos"}
                        {tab.id === "videos" && `${mechVideos.length} vídeos`}
        ...
        {activeTab === "perfil" && (
          <Card>
            <CardHeader><CardTitle>Meu Perfil</CardTitle><CardDescription>Atualize seus dados</CardDescription></CardHeader>
            ...
            {orders.length === 0 ? (
              ...
              <Card key={order.id}>
                <CardContent className="py-4">
                  ...
                  {order.order_items?.map((item: any) => <p key={item.id} className="text-xs text-muted-foreground">{item.quantity}x {item.product_name}</p>)}
                </CardContent>
              </Card>
            ))}
          </div>
                ...
                {catalogs.map((catalog) => (
                  <Card key={catalog.id}>
                    <CardContent className="py-4 flex items-center justify-between">
                      <div><p className="font-semibold text-sm">{catalog.title}</p><p className="text-xs text-muted-foreground">{catalog.category}</p></div>
                ...
                {mechVideos.map((video: any) => (
                  <Card key={video.id}>
                    <CardContent className="py-4">
                      ...
                      {video.description && <p className="text-xs text-muted-foreground mb-2">{video.description}</p>}
                      <a href={video.video_url} target="_blank" rel="noopener noreferrer" className="text-primary text-sm font-medium hover:underline">Assistir →</a>
                    </CardContent>
                  </Card>
                ))}
              </div>
