...
import {
  LayoutDashboard, Package, ShoppingCart, Users, LogOut, Plus, Trash2, Edit, Tag, Eye, EyeOff, Search, ChevronDown, ChevronUp, X
} from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const updateOrderStatus = async (id: string, status: string) => {
    ...
    toast({ title: `Status atualizado para ${statusLabel[status]}` });
    loadAll();
  };
  ...
  return (
    <div className="min-h-screen flex bg-muted">
      {/* Sidebar */}
      <aside className="w-64 bg-sidebar-background text-sidebar-foreground min-h-screen p-4 space-y-1 flex flex-col">
        <h2 className="font-heading text-lg font-bold text-sidebar-primary mb-6 px-3">Painel Admin</h2>
        <nav className="flex-1 space-y-1">
          {sideItems.map((item) => (
            <button key={item.key} onClick={() => { setTab(item.key); setSearchTerm(""); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${tab === item.key ? "bg-sidebar-primary text-sidebar-primary-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"}`}>
              <item.icon className="h-4 w-4" /> {item.label}
              {item.key === "orders" && stats.pendingOrders > 0 && (
                <Badge variant="destructive" className="ml-auto text-[10px] px-1.5 py-0">{stats.pendingOrders}</Badge>
              )}
            </button>
          ))}
        </nav>
        <div className="border-t border-sidebar-border pt-4 space-y-1">
          <button onClick={() => navigate("/")} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
            <ShoppingCart className="h-4 w-4" /> Ver Loja
          </button>
          <button onClick={signOut} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-destructive hover:opacity-80">
      ...
      <main className="flex-1 p-6 overflow-auto">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
              {[
                ...
                { label: "Pedidos", value: stats.totalOrders, icon: ShoppingCart, color: "text-secondary" },
                { label: "Receita Total", value: `R$ ${stats.revenue.toFixed(2).replace(".",",")}`, icon: LayoutDashboard, color: "text-primary" },
                { label: "Pendentes", value: stats.pendingOrders, icon: ShoppingCart, color: "text-accent-foreground" },
                { label: "Clientes", value: stats.totalClients, icon: Users, color: "text-secondary" },
              ].map((s) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-card rounded-xl shadow-sm border border-border">
                <div className="p-5 border-b border-border flex items-center justify-between">
                  <h2 className="font-heading text-lg font-bold">Últimos Pedidos</h2>
              ...
              <div className="bg-card rounded-xl shadow-sm border border-border">
                <div className="p-5 border-b border-border">
                  <h2 className="font-heading text-lg font-bold">Estoque Baixo (≤ 5)</h2>
                </div>
                <div className="divide-y divide-border">
        ...
        {tab === "products" && (
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <Button onClick={() => { setEditingProduct({}); resetProductForm(); }}>
            ...
            {editingProduct !== null && (
              <div className="bg-card rounded-xl shadow-sm border border-border p-6 mb-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-heading text-lg font-bold">{editingProduct.id ? "Editar" : "Novo"} Produto</h3>
                  <button onClick={() => setEditingProduct(null)}><X className="h-5 w-5 text-muted-foreground" /></button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="lg:col-span-2"><Label>Nome do Produto *</Label><Input value={productForm.name} onChange={(e) => setProductForm({ ...productForm, name: e.target.value })} placeholder="Ex: Gerador Diesel 100kVA" /></div>
                  ...
                  <div><Label>Preço Original (R$)</Label><Input type="number" step="0.01" value={productForm.original_price} onChange={(e) => setProductForm({ ...productForm, original_price: e.target.value })} placeholder="Deixe vazio se não há desconto" /></div>
                  ...
                  <div>
                    ...
                    <select className="w-full border border-input rounded-md px-3 py-2 text-sm bg-background" value={productForm.category_id} onChange={(e) => setProductForm({ ...productForm, category_id: e.target.value })}>
                      <option value="">Sem categoria</option>
                      {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                  </div>
                  <div className="lg:col-span-2"><Label>URL da Imagem</Label><Input value={productForm.image_url} onChange={(e) => setProductForm({ ...productForm, image_url: e.target.value })} placeholder="https://..." /></div>
                  <div className="lg:col-span-3"><Label>Descrição</Label><Textarea rows={3} value={productForm.description} onChange={(e) => setProductForm({ ...productForm, description: e.target.value })} placeholder="Descrição detalhada do produto..." /></div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted/50">
                    <tr>
                      ...
                      <th className="text-center p-3 font-semibold">Destaque</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {filteredProducts.map((p) => (
                      <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                        <td className="p-3">
                          <div className="flex items-center gap-3">
                            {p.image_url && <img src={p.image_url} alt="" className="h-10 w-10 rounded object-cover bg-muted" />}
                        ...
                        <td className="p-3 text-right">
                          {p.original_price && <span className="text-muted-foreground line-through text-xs mr-2">R$ {Number(p.original_price).toFixed(2).replace(".",",")}</span>}
                        ...
                        <td className="p-3 text-center">{p.is_active ? <Eye className="h-4 w-4 text-primary mx-auto" /> : <EyeOff className="h-4 w-4 text-muted-foreground mx-auto" />}</td>
                        <td className="p-3 text-center">{p.is_featured ? "⭐" : ""}</td>
                        <td className="p-3">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="icon" onClick={() => editProduct(p)}><Edit className="h-4 w-4" /></Button>
        ...
        {tab === "orders" && (
          <div>
            ...
            <div className="space-y-3">
                ...
                <div key={o.id} className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                  <div className="p-4 flex items-center justify-between cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => loadOrderItems(o.id)}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <select value={o.status} onClick={(e) => e.stopPropagation()} onChange={(e) => updateOrderStatus(o.id, e.target.value)} className="border border-input rounded-md px-2 py-1 text-xs bg-background">
                    ...
                    <div className="border-t border-border p-4 bg-muted/20">
                      {o.profile && (
                        <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                          <div><span className="text-muted-foreground">Cliente:</span> <strong>{o.profile.full_name || "—"}</strong></div>
                      ...
                      <table className="w-full text-sm">
                        <thead><tr className="text-muted-foreground text-xs"><th className="text-left pb-2">Item</th><th className="text-center pb-2">Qtd</th><th className="text-right pb-2">Preço Unit.</th><th className="text-right pb-2">Subtotal</th></tr></thead>
                        <tbody className="divide-y divide-border">
                          {(o.items || []).map(item => (
                            <tr key={item.id}>
                              ...
                              <td className="py-2 text-right font-medium">R$ {(item.quantity * Number(item.price_at_purchase)).toFixed(2).replace(".",",")}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {orders.length === 0 && <p className="text-center text-muted-foreground py-12">Nenhum pedido recebido ainda.</p>}
            </div>
          </div>
        )}
        ...
        {tab === "categories" && (
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <Button onClick={() => { setEditingCategory({}); setCategoryForm({ name: "", slug: "", description: "", image_url: "" }); }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {categories.map(c => (
                <div key={c.id} className="bg-card rounded-xl shadow-sm border border-border p-5">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-2">{products.filter(p => p.category_id === c.id).length} produtos</p>
                    </div>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={() => { setEditingCategory(c); setCategoryForm({ name: c.name, slug: c.slug, description: c.description || "", image_url: c.image_url || "" }); }}><Edit className="h-4 w-4" /></Button>
        ...
        {tab === "clients" && (
          <div>
            <h1 className="font-heading text-3xl font-bold mb-6">Clientes</h1>
            <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-muted/50">
                  ...
                  <tbody className="divide-y divide-border">
                    {clients.map(c => (
                      <tr key={c.user_id} className="hover:bg-muted/30">
                        ...
                        <td className="p-3 text-muted-foreground">{[c.city, c.state].filter(Boolean).join("/") || "—"}</td>
                      </tr>
                    ))}
